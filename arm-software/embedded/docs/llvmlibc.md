...
* `-latomic` to include weak fallback definitions for compiler-emitted
  ...
  for atomic operations that are not lock-free. See the
...
```
clang --config=llvmlibc.cfg --target=arm-none-eabi -march=armv7m  -nostartfiles -lcrt0-semihost -lsemihost -latomic -T llvmlibc.ld -o hello hello.c
```
