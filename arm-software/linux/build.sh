...
package() {
    ...
    if ! libraries_present; then
      echo "-mllvm -mllvm -store-to-load-forwarding-conflict-detection=0" > atfl-performance.cfg
    else
      echo "-fveclib=ArmPL -mllvm -mllvm -store-to-load-forwarding-conflict-detection=0" > atfl-performance.cfg
    fi
