...
product_build() {
    ...
    local bootstrap_builtins="$(
        "${BOOTSTRAP_COMPILER_DIR}/bin/clang++" \
             --target="${ATFL_TARGET_TRIPLE}" \
             --rtlib=compiler-rt \
             --print-libgcc-file-name
    )"
    if [[ ! -f "${bootstrap_builtins}" ]]; then
      echo "error: bootstrap compiler-rt builtins not found: ${bootstrap_builtins}" >&2
      return 1
    fi
    echo "Using compiler-rt builtins: ${bootstrap_builtins}"
    ...
    run_command cmake "${CMAKE_ARGS[@]}" -G Ninja "${SOURCES_DIR}/llvm" \
        ...
        -DRUNTIMES_CMAKE_ARGS="-DCOMPILER_RT_USE_BUILTINS_LIBRARY=ON;-DCOMPILER_RT_LIBRARY_builtins_${ATFL_TARGET_TRIPLE}=${bootstrap_builtins};-DCMAKE_CXX_FLAGS=-stdlib++-isystem${ATFL_DIR}/include/c++/v1 -D_LIBCPP_VERBOSE_ABORT_NOT_NOEXCEPT;-DCMAKE_EXE_LINKER_FLAGS=${rtlibs};-DCMAKE_MODULE_LINKER_FLAGS=${rtlibs};-DCMAKE_SHARED_LINKER_FLAGS=${rtlibs}" \
